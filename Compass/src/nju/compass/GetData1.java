...
public class GetData1 extends java.util.TimerTask {
	...
	public void run() {
		...
		for (int i = 0; i < 6; i++) {
			...
			n = dftime[i];
			R = dollarloan[i];
			rowData[rowNumber][3] = df.format((A * (1 + r * n / 12) - B * (1 + R * n / 12)));
		...
		for (int i = 0; i < 6; i++) {
			...
			m = dftime[j];
			R = dollarloan[i];
			rowData[rowNumber][3] = df.format(( B * (1 + r * (m - n) / 12) - C*(1 + R * (m - n) / 12)));
		...
		for (int i = 0; i < 6; i++) {
			...
			m = dftime[j];
			R = dollardeposit[i];
			rowData[rowNumber][3] = df.format( ( C * (1 + R * (m - n) / 12) - B*(1 + r * (m - n) / 12)));
